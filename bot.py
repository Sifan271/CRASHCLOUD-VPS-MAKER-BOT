...
TOKEN = 'MTM2MTQzMDY5ODUzMTIyNTcwNg.G5qjWr.5YvDSLhXcx_xjF9RzsFQJkvbomrYDS0lvdib0Y'
...
whitelist_ids = {"1139926248181747782"}  # Replace with actual user IDs
...
async def create_server_task(interaction):
    await interaction.response.send_message(embed=discord.Embed(description="### Creating Instance, This takes a few seconds. Powered by [GhostVps](<https://discord.gg/QQVpm3ycsQ)", color=0x00ff00))
    userid = str(interaction.user.id)
    ...
    try:
        container_id = subprocess.check_output([
           "docker", "run", "-itd", "--privileged", "--hostname", "GhostVps", "--cap-add=ALL", image
        ]).strip().decode('utf-8')
    except subprocess.CalledProcessError as e:
